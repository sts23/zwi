...
do
  ...
  if [ "$MUSTER" = "" ]
  then
    SNAPS=$(zfs list -r -t snap -H -o name -s creation -d1 "${DATASET}")
  else
    SNAPS=$(zfs list -r -t snap -H -o name -s creation -d1 "${DATASET}"|grep $MUSTER)
  fi
  ...
  then
      ...
      then
        ...
        if [ "$MUSTER" = "" ]
        then
          zfs list -r -t snap -o name -s creation -d 1 "${DATASET}"
        else
          zfs list -r -t snap -o name -s creation -d 1 "${DATASET}"|grep $MUSTER
        fi
      else
